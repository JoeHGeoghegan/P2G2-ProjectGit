...
def stock_separate(stocks:pd.DataFrame):
    stockDict = {}
    for ticker in stocks['ticker'].unique():
        stockDict[ticker] = stocks[stocks['ticker']==ticker]
    return stockDict
def load_stocks(path='.\Data\Cleaned_Data\stock_data.csv'):
    stocks = pd.read_csv(path,index_col=0)
    return stock_separate(stocks)
